...
public class Users {
	...
	public Map<String, Object> editUsuario(@RequestBody Map<String, Object> usuarioData, @PathVariable("id") long id) {
		String sql = "UPDATE public.usuarios SET nombre = '%s', apellido = '%s', telefono = '%s', direccion = '%s', id_rol = %d,activo=%s, recibir_notificaciones=%b, id_ciudad=%d WHERE id = %d RETURNING *";
		sql = String.format(sql, usuarioData.get("nombre"), usuarioData.get("apellido"), usuarioData.get("telefono"),
				usuarioData.get("direccion"), usuarioData.get("id_rol"), usuarioData.get("activo"),usuarioData.get("recibir_notificaciones"),usuarioData.get("id_ciudad") , id);
